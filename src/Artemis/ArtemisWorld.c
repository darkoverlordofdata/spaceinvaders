 ...
 */
void ArtemisWorldInitialize(ArtemisWorldRef this)
{
...
{
    CFMapSet(this->managers, CFStringCreate(manager->cls->name), manager);
...
{
    return CFMapGet(this->managers, CFStringCreate(cls->name));
}
...
{
    CFMapSet(this->managers, CFStringCreate(manager->cls->name), NULL);
 ...
 */
ArtemisEntitySystemRef ArtemisWorldSetSystem(ArtemisWorldRef this, ArtemisEntitySystemRef system, bool passive)
{
    CFObjectRef obj = (CFObjectRef)system;

    ArtemisEntitySystemSetWorld(system, this);
    ArtemisEntitySystemSetPassive(system, passive);
    CFMapSet(this->systems, CFStringCreate(obj->cls->name), system);
...
{
    CFObjectRef obj = (CFObjectRef)system;
    CFMapSet(this->systems, CFStringCreate(obj->cls->name), NULL);
    CFBagRemove(this->systemsBag, system);
}

void ArtemisWorldNotifySystems(ArtemisWorldRef this, ArtemisPerform perform, ArtemisEntityRef e)
{
    for (ulong i=0; i<CFBagSize(this->systemsBag); i++) {
...
void ArtemisWorldNotifyManagers(ArtemisWorldRef this, ArtemisPerform perform, ArtemisEntityRef e)
{
    for (ulong i=0; i<CFBagSize(this->managersBag); i++) {
 ...
 * @param performer
 */
void ArtemisWorldCheck(ArtemisWorldRef this, CFBagRef entities, ArtemisPerform performer)
{
